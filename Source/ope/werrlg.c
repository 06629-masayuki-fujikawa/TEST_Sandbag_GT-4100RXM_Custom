...
// MH321800(E) Y.Tanizaki ICクレジット対応 不要機能削除(Edy)
	...
	if( moj == mod_flapcrb &&											// ロック装置エラー
		...
		if( 0 == LKcom_RoomNoToType(Err_work.ErrBinDat) ) {							// エラーの車室はロック装置
			if(prm_get( COM_PRM, S_PRN, 19, 1, 1 ) == 1 ){							// 18-0019⑥=1　上昇/下降及び開/閉ロック印字する
				Err_work.Errinf = 2;												// 付属ﾃﾞｰﾀ(bin)あり
			}
		}
		else { 																		// エラーの車種はフラップ
			if(prm_get( COM_PRM, S_PRN, 19, 1, 3 ) == 1 ){							// 18-0019④=1　上昇/下降及び開/閉ロック印字する
				Err_work.Errinf = 2;												// 付属ﾃﾞｰﾀ(bin)あり
			}
		}
// MH322914(S) K.Onodera 2016/09/07 AI-V対応：エラーアラーム
...
// MH322917(S) A.Iiizumi 2018/09/03 長期駐車検出機能の拡張対応(検出部分)
				if(prm_get( COM_PRM, S_PRN, 33, 1, 1 ) != 0 ){	// 18-0033⑥長期駐車アラームジャーナル印字 しない
...
// 不具合修正(E) K.Onodera 2016/10/13 #1505 アラーム01-61(5000円札釣り切れ)発生を含むデータが応答されてしまう
	...
	for( ; sno < smax; sno++ ){
		...
		if( ucErrWk ){
			...
			// 13/4/4追加(S)
			if( 0 == LKcom_RoomNoToType(Err_work.ErrBinDat) ){					// エラーの車室はロック装置
				if( prm_get( COM_PRM, S_PRN, 19, 1, 1 ) == 0 ){					// 18-0019⑥=1　上昇/下降及び開/閉ロック印字しない
					Err_work.Errinf = 0;										// 付属ﾃﾞｰﾀ(bin)なしとする
				}
			} else { 															// エラーの車種はフラップ
				if( prm_get( COM_PRM, S_PRN, 19, 1, 3 ) == 0 ){					// 18-0019④=1　上昇/下降及び開/閉ロック印字しない
					Err_work.Errinf = 0;										// 付属ﾃﾞｰﾀ(bin)なしとする
				}
			}
			// 13/4/4追加(E)
